...
import 'package:zezis_printing/pdf/widgets.dart' as pw;

/// ImageProvider that draws a Flutter Widget on a PDF document
class ZWrapper extends pw.ImageProvider {
  ZWrapper._(
    this.bytes,
    int width,
    int height,
  ...
  ///
  ///   final image = await ZWrapper.fromKey(key: rb);
  ///
  ///   pdf.addPage(
  ///     pw.Page(
  ///       build: (context) {
  ...
  /// }
  /// ```
  static Future<ZWrapper> fromKey({
  ...
  ///
  /// ```
  /// final wrapped = await ZWrapper.fromWidget(
  ///   widget: Container(
  ///     color: Colors.white,
  ///     child: Text(
  ///       'Hello world !',
  ...
  /// );
  /// ```
  static Future<ZWrapper> fromWidget({
